...
(function migrate() {
  ...
  try {
    ...
    if (migrated > 0)
      console.log(
        `[Storage] Migrated ${migrated} recording(s) from file storage to SQLite.`,
      );
  } catch (err) {
...
function dbAllMeta() {
  return stmts.allMeta.all().map(parseMeta);
}
function dbGetMeta(id) {
  return parseMeta(stmts.getMeta.get(id));
}
...
function dbDeleteRecording(id) {
  stmts.deleteRecording.run(id);
}

function dbGetHistory(recId) {
  return stmts.getHistory.all(recId);
}
...
// Build a lookup map: fullUrl → [response objects]. Same URL called N times
// keeps N entries so compareResponses can pair occurrences positionally.
...
function jsonDiff(expected, actual, path) {
  ...
  if (Array.isArray(expected) && Array.isArray(actual)) {
    ...
    if (expected.length !== actual.length)
      diffs.push(
        `${path}[]: 길이 변경 (${expected.length} → ${actual.length})`,
      );
  ...
  if (typeof expected === "object") {
    ...
    for (const k of keys) {
      if (!(k in expected)) {
        diffs.push(`${path}.${k}: 키 추가됨`);
        continue;
      }
      if (!(k in actual)) {
        diffs.push(`${path}.${k}: 키 삭제됨`);
        continue;
      }
      ...
      if (rec.body !== null) {
        if (act.body === null) {
          ...
          try {
            ...
            const actJson = JSON.parse(act.body);
...
app.put("/api/recordings/:id/responses", (req, res) => {
  ...
  db.prepare(`UPDATE recordings SET responses = ? WHERE id = ?`).run(
...
function generateScript(rec) {
  ...
  for (const ev of rec.events) {
    ...
    switch (ev.type) {
      case "navigate":
        lines.push(
          `  await page.goto(${JSON.stringify(ev.url)}, { waitUntil: 'domcontentloaded' });`,
        );
        break;
      case "click":
        lines.push(
      ...
      case "scroll":
        lines.push(
          `  await page.evaluate(() => window.scrollTo(${ev.scrollX}, ${ev.scrollY}));`,
        );
        break;
      case "keydown":
      ...
      case "input":
        ...
        lines.push(
          `  await page.keyboard.type(${JSON.stringify(ev.value ?? "")});`,
        );
        break;
    }
  }
